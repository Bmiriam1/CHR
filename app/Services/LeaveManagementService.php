...
class LeaveManagementService
{
    ...
    {
        $user = User::find($requestData['user_id'] ?? Auth::id());
        ...
        // Get company_id from program if not provided
        $program = Program::find($requestData['program_id']);
        $companyId = $requestData['company_id'] ?? $program->company_id;
        ...
        $leaveRequest = LeaveRequest::create([
            ...
            'leave_type_id' => $leaveType->id,
            'leave_type' => $this->mapLeaveTypeToEnum($leaveType->code), // Keep for backward compatibility
    ...
    public function getLeaveBalanceSummary(User $user, ?int $year = null): array
    {
        ...
        // Create balances array for view compatibility
        $balances = [];
        foreach ($accrualSummary as $accrual) {
            $leaveTypeCode = strtolower($accrual['leave_type_code']);
            $balances[$leaveTypeCode] = [
                'current' => $accrual['current_balance'],
                'accrued_this_year' => $accrual['total_accrued'],
                'taken_this_year' => $accrual['total_deducted'],
                'carry_over' => 0, // Will be populated from carry over summary if needed
            ];
        }
        ...
        return [
            ...
            'service_info' => [
                ...
                'is_probation' => $balance->is_probationary,
    ...
    private function getDailyRate(User $user): float
    {
        ...
        $program = $user->programs()->where('programs.status', 'active')->first();
    ...
    /**
     * Map leave type code to enum value for backward compatibility.
     */
    private function mapLeaveTypeToEnum(string $leaveTypeCode): string
    {
        $mapping = [
            ...
            'MATERNITY' => 'personal',
            'PATERNITY' => 'personal',
            'FAMILY_RESPONSIBILITY' => 'personal',
            'STUDY' => 'personal',
            'EMERGENCY' => 'emergency',
            'OTHER' => 'other',
        ];
