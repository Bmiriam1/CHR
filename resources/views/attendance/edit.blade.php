                    ...
                    <form method="POST" action="{{ route('attendance.update', $attendance) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="program_id" id="program_id" required
                                    ...
                                    @foreach($programs as $program)
                                        <option value="{{ $program->id }}" {{ (old('program_id', $attendance->program_id) == $program->id) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                    ...
                                    <option value="absent_unauthorized" {{ (old('status', $attendance->status) == 'absent_unauthorized') ? 'selected' : '' }}>Absent
                                        (Unauthorized)</option>
                                    <option value="absent_authorized" {{ (old('status', $attendance->status) == 'absent_authorized') ? 'selected' : '' }}>Absent
                                        (Authorized)</option>
                            ...
                            <div>
                                ...
                                <input type="time" name="check_in_time" id="check_in_time"
                                    value="{{ old('check_in_time', $attendance->check_in_time?->format('H:i')) }}"
                            ...
                            <div>
                                ...
                                <input type="time" name="check_out_time" id="check_out_time"
                                    value="{{ old('check_out_time', $attendance->check_out_time?->format('H:i')) }}"
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="exception_reason" id="exception_reason" rows="3"
                                ...
                                placeholder="Reason for absence or exception">{{ old('exception_reason', $attendance->exception_reason) }}</textarea>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                ...
                                placeholder="Additional notes or comments">{{ old('notes', $attendance->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
